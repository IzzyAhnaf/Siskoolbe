...
const CheckoutGuru = () => {
  ...
  const absen = _debounce(async () => {
    try{
      const resp = await api.post('/absensikeluarguru', {id}, {headers: {Authorization: `${sessionStorage.getItem("token")}`}})
      if(resp.status === 200){
        Swal.fire(
          'Berhasil',
          'Absen Berhasil',
  ...
  const absenChecker = _debounce(async () => {
    try{
      const resp = await api.get(`/absensikeluarCheckerGuru/${id}`, {headers: {Authorization: `${sessionStorage.getItem("token")}`}})
      if(resp.status === 200){
  ...
  return (
    <>
      {!WMobile ? (
        defaultCenter && (
          <div className='flex flex-col w-full rounded-lg items-center justify-center h-full bg-[#D9D9D9] mx-4'>
            <div className="flex bg-sky-700 mt-4 w-11/12 h-16 mx-10 rounded-3xl items-center px-2 py-2"
            ...
            className='bg-[#FF0000] text-white rounded-md py-2 mt-5 text-base text-center w-11/12 mx-10'>
              Absen Keluar
            </button>
          </div>
        )
